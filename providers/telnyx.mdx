...
description: "Connect your Telnyx account and messaging profile to send and receive SMS through EzyText."
icon: "plug"
---
...
Telnyx is one of two SMS providers EzyText supports (the other is
[Twilio](/providers/twilio)). Connecting Telnyx lets EzyText send text messages
...
<Note>
  ...
  plays in Twilio. EzyText uses "Messaging Profile" terminology throughout the
  Telnyx setup.
</Note>
...
<Warning>
  ...
  Telnyx but can't send messages, and EzyText will reject it during validation.
</Warning>
...
<Steps>
  ...
  <Step title="Enter your credentials">
    ...
    EzyText validates format live — the API Key must start with `KEY`, and the
    Messaging Profile ID (if provided) must be a valid UUID.
  </Step>
  <Step title="Connect & save">
    Click **Connect & Save Credentials**. EzyText calls Telnyx to list your
...
Webhooks are configured automatically. EzyText sets your messaging profile's
...
<AccordionGroup>
  <Accordion title="Do I need a Messaging Profile ID to connect?">
    No — it's optional. If you leave it blank, EzyText discovers your messaging
  ...
  <Accordion title="What's the difference between a profile and a service?">
    ...
    profile**; Twilio calls it a **messaging service**. EzyText assigns numbers to
...
<AccordionGroup>
  ...
  <Accordion title="Numbers don't receive replies">
    Finish setup so EzyText can write the webhook URL to your messaging profile, and
